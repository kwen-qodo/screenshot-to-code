...
@pytest.mark.asyncio
@patch("backend.llm.AsyncDeepSeek")
async def test_stream_deepseek_response_happy_path(mock_deepseek):
    mock_client = mock_deepseek.return_value
    mock_stream = AsyncMock()
    mock_stream.__aiter__.return_value = [type("Chunk", (), {"content": "DeepSeek response"})()]
    mock_client.chat.completions.create.return_value = mock_stream
    callback, results = make_callback()
    messages = [{"role": "system", "content": "You are helpful."}, {"role": "user", "content": "Hi"}]
    out = await stream_deepseek_response(messages, "fake-key", callback, Llm.GPT_4O_2024_05_13)
    assert "duration" in out and "code" in out
    assert results and "DeepSeek response" in results[0]

@pytest.mark.asyncio
@patch("backend.llm.AsyncDeepSeek")
async def test_stream_deepseek_response_edge_case_invalid_key(mock_deepseek):
    mock_client = mock_deepseek.return_value
    mock_client.chat.completions.create.side_effect = Exception("Invalid key")
    callback, _ = make_callback()
    messages = [{"role": "system", "content": "You are helpful."}, {"role": "user", "content": "Hi"}]
    with pytest.raises(Exception):
        await stream_deepseek_response(messages, "bad-key", callback, Llm.GPT_4O_2024_05_13)
